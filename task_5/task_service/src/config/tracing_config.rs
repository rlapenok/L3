...
use opentelemetry::{global::{set_error_handler, set_text_map_propagator}, trace::TracerProvider, KeyValue};
use opentelemetry_otlp::{new_exporter,new_pipeline,WithExportConfig};
use opentelemetry_sdk::{propagation::TraceContextPropagator, runtime::Tokio, trace::{Config as TraceConfig, Tracer}, Resource};
use tracing::{error,Level};
use tracing_opentelemetry::{layer as otel_layer, OpenTelemetryLayer};
use tracing_subscriber::{filter::{Directive, Filtered}, fmt::  Layer, layer::SubscriberExt, util::SubscriberInitExt, EnvFilter, Layer as LayerTrait, Registry};
...
impl TracingConfig {
    ...
    fn create_otel_layer(&self)->Result<OtelFiltered, Box<dyn Error>>{
        ...
        set_error_handler(|err|{
            error!("Error while send trace to GrafanaTempo:{}", err)
        })?;
        let key_value = vec![KeyValue::new("service.name", "TaskService")];
    ...
    pub fn run_sub(&self)->Result<(),Box<dyn Error>>{
        ...
        let stdout_layer=Layer::new().with_file(false).compact().with_target(false).with_filter(stdout_filters);
